...
public final class ExecutorServiceImpl
    ...
    private static final TraceComponent tc = Tr
            .register(ExecutorServiceImpl.class);
    ...
    @Deprecated
    protected synchronized void setDeprecatedInterceptor(
            ExecutorServiceTaskInterceptor interceptor) {
        interceptors.add(interceptor);
        interceptorsActive = true;
    }

    @Deprecated
    protected synchronized void unsetDeprecatedInterceptor(
            ExecutorServiceTaskInterceptor interceptor) {
    ...
    public static boolean isBeta = Boolean
            .valueOf(System.getProperty("com.ibm.ws.beta.edition"));
    ...
    private synchronized void createExecutor() {
        if (componentConfig == null) {
            // this is a completely normal occurrence and can happen if a
            // ThreadFactory is
            // bound prior to
            // component activation... the proper thing to do is to do nothing
            // and wait for
            // activation
            return;
        }
        ...
        int coreThreads = Integer
                .parseInt(String.valueOf(componentConfig.get("coreThreads")));
        int maxThreads = Integer
                .parseInt(String.valueOf(componentConfig.get("maxThreads")));
        if (isBeta) {
            String quiesceTimeoutString = (String) (componentConfig
                    .get("quiesceTimeout"));
            try {
                quiesceTimeout = Integer.valueOf(KernelUtils
                        .parseDuration(quiesceTimeoutString, TimeUnit.SECONDS));
            } catch (NumberFormatException nfe) {
                Tr.warning(tc, "CWWKE1206.quiesce.timeout.not.valid",
                        quiesceTimeoutString);
            }
        }
        ...
        coreThreads = Math.max(MINIMUM_POOL_SIZE,
                Math.min(coreThreads, maxThreads));
        // ... and then make sure maxThreads is not smaller than coreThreads ...
        maxThreads = Math.max(coreThreads, maxThreads);

        BlockingQueue<Runnable> workQueue = new BoundedBuffer<Runnable>(
                java.lang.Runnable.class, 1000, 1000);

        RejectedExecutionHandler rejectedExecutionHandler = new ExpandPolicy(
                workQueue, this);

        if (threadPool != null) {
            ((BoundedBuffer<Runnable>) threadPool.getQueue())
                    .removeFromAvailableProcessors();
        }
        threadPool = new ThreadPoolExecutor(coreThreads, maxThreads, 0,
                TimeUnit.MILLISECONDS, workQueue,
                threadFactory != null
                        ? threadFactory
                        : new ThreadFactoryImpl(poolName, threadGroupName),
    ...
    private void softShutdown(final ThreadPoolExecutor oldThreadPool) {
        // setting keepAlive and coreThreads to 0 ensures that idle threads go
        // away
        // immediately; the
        // thread pool will be eligible for garbage collection when all threads
        // are gone
        // and no other
    ...
    public static class ExpandPolicy implements RejectedExecutionHandler {
         ...
         */
        public ExpandPolicy(BlockingQueue<Runnable> workQueue2,
                WSExecutorService exService) {
    ...
    public boolean quiesceThreads() {
        ...
        try {
            ...
            phaser.arriveAndDeregister();
            phaser.awaitAdvanceInterruptibly(0, quiesceTimeout,
                    TimeUnit.SECONDS);
    ...
    public void setAvailableProcessors(int availableProcessors) {
        if (componentConfig != null) {
            int coreThreads = Integer.parseInt(
                    String.valueOf(componentConfig.get("coreThreads")));
            if (coreThreads < 0) {
                // Create a new executor; the number of cpus available to the
                // process has
                // changed
                // which causes the algorithms used by ThreadPoolExecutor to
                // change
                createExecutor();
            }
        }
