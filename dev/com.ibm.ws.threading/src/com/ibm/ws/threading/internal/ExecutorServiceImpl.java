 ...
 */
@Component(name = "com.ibm.ws.threading", configurationPolicy = ConfigurationPolicy.REQUIRE, property = "service.vendor=IBM", service = {
		ExecutorService.class, WSExecutorService.class })
public final class ExecutorServiceImpl implements WSExecutorService, ThreadQuiesce, AvailableProcessorsListener {
	...
	/**
	 * The controller that (when active) can monitor the throughput of the
	 * underlying thread pool and adjust its size in an attempt to maximize
	 * throughput.
	 */
	...
	/**
	 * The smallest size for the pool - smaller coreThreads and/or maxThreads config
	 * values are replaced with this value.
	 */
	final static int MINIMUM_POOL_SIZE = 4;

	/**
	 * Amount of time to wait for quiesce work to complete before continuing with
	 * shutdown.
	 */
	 ...
	 * performance reasons, to avoid getting an iterator over an empty set for every
	 * task that is submitted.
	 */
	boolean interceptorsActive = false;

	/**
	 * A Set of interceptors that are all given a chance to wrap tasks that are
	 * submitted to the executor for execution.
	 */
	Set<TaskInterceptor> interceptors = new CopyOnWriteArraySet<>();

	@Reference(cardinality = ReferenceCardinality.MULTIPLE, policy = ReferencePolicy.DYNAMIC)
	@Deprecated
	protected synchronized void setDeprecatedInterceptor(ExecutorServiceTaskInterceptor interceptor) {
		interceptors.add(interceptor);
		interceptorsActive = true;
	}

	@Deprecated
	protected synchronized void unsetDeprecatedInterceptor(ExecutorServiceTaskInterceptor interceptor) {
	...
	@Reference(cardinality = ReferenceCardinality.MULTIPLE, policy = ReferencePolicy.DYNAMIC)
	protected synchronized void setInterceptor(TaskInterceptor interceptor) {
		interceptors.add(interceptor);
		interceptorsActive = true;
	}

	protected synchronized void unsetInterceptor(TaskInterceptor interceptor) {
		interceptors.remove(interceptor);
		if (interceptors.size() == 0) {
			interceptorsActive = false;
		}
	...
	private synchronized void createExecutor() {
		if (componentConfig == null) {
			// this is a completely normal occurrence and can happen if a ThreadFactory is
			// bound prior to
			// component activation... the proper thing to do is to do nothing and wait for
			// activation
			return;
		}
		...
		// Make sure coreThreads is not bigger than maxThreads, subject to
		// MINIMUM_POOL_SIZE limit
		...
		threadPool = new ThreadPoolExecutor(coreThreads, maxThreads, 0, TimeUnit.MILLISECONDS, workQueue,
	 ...
	 * delegates to. This allows the proxy Runnable to be offered directly to the
	 * BlockingQueue, which can then identify information about it, such as whether
	...
	/**
	 * Shutdown a thread pool while still allowing current users to submit new work
	 * to it. The standard ThreadPoolExecutor.shutdown() method causes any new work
	 ...
	 * shutting down. The thread pool will stay alive until no more references to it
	...
	private void softShutdown(final ThreadPoolExecutor oldThreadPool) {
		// setting keepAlive and coreThreads to 0 ensures that idle threads go away
		// immediately; the
		// thread pool will be eligible for garbage collection when all threads are gone
		// and no other
	...
	Runnable wrap(Runnable r) {
		...
		WithContext wc = Optional.of(r).filter(WithContext.class::isInstance).map(WithContext.class::cast)
				.orElse(Optional::empty);
		// TODO Check that the 3rd argument can be null
		return interceptors.stream().sequential().reduce(r, (r2, i) -> i.wrapWithContext(r2, wc), null);
	}
	...
	<T> Callable<T> wrap(Callable<T> c) {
		WithContext wc = Optional.of(c).filter(WithContext.class::isInstance).map(WithContext.class::cast)
				.orElse(Optional::empty);
		// TODO Check that the 3rd argument can be null
		return interceptors.stream().sequential().reduce(c, (c2, i) -> i.wrapWithContext(c2, wc), null);
	}

	// This is private, so handling both interceptors and wrapping in this method
	// for simplicity
	...
	public void setAvailableProcessors(int availableProcessors) {
		if (componentConfig != null) {
			int coreThreads = Integer.parseInt(String.valueOf(componentConfig.get("coreThreads")));
			if (coreThreads < 0) {
				// Create a new executor; the number of cpus available to the process has
				// changed
